...
- 修复独立打包 CLI 在没有 GUI 环境缓存时无法启动 `RIFE -> NV-VFX` 的问题：
  仅在明确需要时按需扫描 CUDA PyTorch / nvvfx 环境，显式 `--torch-python`
  可直接使用，错误和未预期异常会保留在交互窗口中供用户阅读。
- v0.7.0 的 Full/Lite 资产已用提交 `9cd9088` 的修复构建原地替换；Win7 资产不变。
