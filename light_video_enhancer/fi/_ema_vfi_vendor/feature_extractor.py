...
class MotionFormerBlock(nn.Module):
    ...
    def forward(self, x, cor, H, W, B):
        ...
        if shift_mask is not None:
            shift_mask = shift_mask.to(device=x_pad.device, dtype=x_pad.dtype)
...
class MotionFormer(nn.Module):
    ...
    def get_cor(self, shape, device, dtype):
        k = (str(shape), str(device), str(dtype))
        if k not in self.cor:
            ...
            self.cor[k] = torch.cat([tenHorizontal, tenVertical], -1)
        return self.cor[k]
