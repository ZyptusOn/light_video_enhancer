...
public static class Localization
{
    private static readonly IReadOnlyDictionary<string, string> ZhToEn =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["DLoRAL 4×（实验性重型修复）"] = "DLoRAL 4x (experimental heavy restoration)",
            ["OSDEnhancer 4×/2×（联合重型修复）"] = "OSDEnhancer 4x/2x (joint heavy restoration)",
            ["不改变分辨率"] = "Keep original resolution", ["不插帧"] = "No interpolation",
            ["RIFE AI（PyTorch）"] = "RIFE AI (PyTorch)",
            ["EMA-VFI Small（PyTorch CUDA）"] = "EMA-VFI Small (PyTorch CUDA)",
            ["VFIMamba S / Full（PyTorch CUDA）"] = "VFIMamba S / Full (PyTorch CUDA)",
            ["SparkVSR Stage-2 4×（关键帧重型修复）"] = "SparkVSR Stage-2 4x (heavy keyframe restoration)",
            ["DIS 稠密光流"] = "DIS dense optical flow",
