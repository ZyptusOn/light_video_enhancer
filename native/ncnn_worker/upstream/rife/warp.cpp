...
int Warp::create_pipeline(const Option& opt)
{
    ...
    // pack8
    // Current ncnn Vulkan tensors use pack1/pack4 and removed the legacy
    // use_shader_pack8 option.  Do not compile the obsolete shader: its old
    // afpvec8 helpers are intentionally absent from modern ncnn.
    if (false)
    {
        static std::vector<uint32_t> spirv;
        static ncnn::Mutex lock;
