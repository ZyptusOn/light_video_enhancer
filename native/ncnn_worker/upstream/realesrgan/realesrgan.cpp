    ...
    // initialize preprocess and postprocess pipeline
    {
            ...
            if (spirv.empty())
            {
                if (tta_mode)
                    compile_spirv_module(
                        realesrgan_preproc_tta_comp_data,
                        sizeof(realesrgan_preproc_tta_comp_data), net.opt, spirv);
                else
                    compile_spirv_module(
                        realesrgan_preproc_comp_data,
                        sizeof(realesrgan_preproc_comp_data), net.opt, spirv);
            }
...

        {
            ...
            if (spirv.empty())
            {
                if (tta_mode)
                    compile_spirv_module(
                        realesrgan_postproc_tta_comp_data,
                        sizeof(realesrgan_postproc_tta_comp_data), net.opt, spirv);
                else
                    compile_spirv_module(
                        realesrgan_postproc_comp_data,
                        sizeof(realesrgan_postproc_comp_data), net.opt, spirv);
            }
